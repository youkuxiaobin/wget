...
main (int argc, char **argv)
{
  ...
  //noclobber 表示如果当前目录存在这个文件的话，就不会下载这个文件, convert_links 表示下载的html中的链接文件，如果下载过，就会修改为相对html的目录，如果没有下载过，就修改为http://host:port/path
  ...
  //递归层次，如果递归层次为0.那么就设置为无线递归
  if (opt.reclevel == 0)
      opt.reclevel = INFINITE_RECURSION; /* see recur.h for commentary */

  //sider 表示并不下载这些文件，仅仅检查这些文件在哪,打印状态码之类的.opt.delete_after表示下载完之后，对文件进行删除
  if (opt.spider || opt.delete_after)
      opt.no_dirstruct = true;

  //下载页面所需要的显示连接，比如css img
  ...
  //开启时间戳，如果服务器文件比当先下载文件新，就会下载(-N)，否则不进行下载. opt.noclobber表示如果当前目录存在这个文件，就不会继续下载
...
for details.\n\n"));
  ...
  if (opt.verbose)
    //设置进程实现方式，显示下载是滚动bar还是dot显示给client 。progress_type {bar, dot}这两种
    set_progress_implementation (opt.progress_type);//此函数主要是设置current_impl应该是为了后面做准备
  ...
  /* Initialize logging.  */
  log_init (opt.lfilename, append_to_log);//打开日志文件，如果没有文件名，就输出到标准输出，如果有文件名，就追加到这个日志文件
