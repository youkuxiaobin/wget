         ...
         struct iri *iri, int count)
{
  ...
  //如果说本地有文件，那么就从这个本地文件开始下载，如果没有文件，就从0开始下载,这个也就完成了断续下载。
  ...
  //设置http的agent
  SET_USER_AGENT (req);
  //设置http可接受的类型 image/gif image/jpeg之类的 
           ...
           struct iri *iri)
{
  ...
  *newloc = NULL;
  ...
  //保存文件名称 首先是通过 --output-document 如果没有就获取url后缀名称
